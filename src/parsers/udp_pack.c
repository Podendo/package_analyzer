...
static inline struct protocol_eth
*udp_pack_eth_from_raw(struct udp_pack_data *udp_pack_data)
{
...
int udp_pack_init(char *filename, struct udp_pack_data *udp_pack_data)
{
	...
	if ( (udp_pack_data->raw_data == NULL) &&
			(udp_pack_data->ntoh_data == NULL) ) {
...
void udp_pack_print(struct udp_pack_data *udp_pack_data)
{
	...
	printf("\nudp source address:\t%0d ", udp->source_addr);
	printf("\nudp destination address:\t%0d", udp->destination_addr);

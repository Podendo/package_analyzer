...
###############################################################################
# This function is a draft, script does not work with it, use filtering_all()
# 
# function filtering_udp()
# {
# if [ -z "$ARR_UDP_FILTERS" ]; then
#    err "There is no Ethernet filters!"
#    exit 1
#  else
#    echo "Trying to find Ethernet matches"
#  fi
#
#  sleep 1s
#  touch $FILE_TMP
...
#  done
#
#  FILTERS=${ARR_UDP_FILTERS[@]/#/-e }
#  for file in ${ARR_FILES[@]:0:$PACK_NUM};
#  do
#    $ELF_FILE $file > $FILE_TMP
#    wait $!
#    if grep -q -i "udp" $FILE_TMP && grep -i $FILTERS $FILE_TMP; then
#      ARR_MATCHES=(${ARR_MATCHES[@]} "$file")
#      echo "match in file $file"
#      echo
#    else
...
#  done
#  sleep 1s
#  echo "udp filtering result packages: ${ARR_MATCHES[@]}"
#  rm $FILE_TMP
#}
